...
class CustomEditText : androidx.appcompat.widget.AppCompatEditText {
    ...
    init {
//        super.setTypeface(ResourcesCompat.getFont(context, R.font.frutiger_roman))
    }
        ...
        when (validationType) {
            1 -> { //email
                if (!Validation.isEmailValid(text?.toString()!!)) {
                    error = validationErrorMsg ?: context.getString(R.string.email_validation_error_msg)
                    return false
                }
            }
            2 -> { //phone
                if (!Validation.isValidPhone(text?.toString()!!)) {
                    error = validationErrorMsg ?: context.getString(R.string.phone_validation_error_msg)
                    return false
                }
            }
            3 -> { // password
            ...
            4 -> { //confirm password
                ...
                if (!Validation.isPasswordsMatches(
                        text?.toString()!!,
                        rootView.findViewById<CustomEditText>(mustMatched).text?.toString()!!
                    )
                ) {
                    error = validationErrorMsg ?: context.getString(R.string.confirm_pass_validation_error_msg)
                    return false
                }
            }
            5 -> { //phone or email
                if (!Validation.isValidPhone(text?.toString()!!) && !Validation.isEmailValid(text?.toString()!!)) {
                    error = validationErrorMsg ?: context.getString(R.string.phone_or_email_validation_error_msg)
                    return false
                }
            }
            6 -> { // egyptian phone
            ...
            7 -> { // egyptian phone or email
                if (!Validation.isValidPhoneEgypt(text?.toString()!!) && !Validation.isEmailValid(text?.toString()!!)) {
                    error = validationErrorMsg ?: context.getString(R.string.phone_or_email_validation_error_msg)
                    return false
                }
            }
            8 -> { // name
                if (!Validation.isValidName(text?.toString()!!) && !Validation.isEmailValid(text?.toString()!!)) {
                    error = validationErrorMsg ?: context.getString(R.string.name_validation_error_msg)
                    return false
                }
            }
        }
